...
public class PositiveLoginServiceTest {
	@InjectMocks
    ...
    private static final Logger LOG = Logger.getLogger(PositiveLoginServiceTest.class);
...
//	 Mockito.reset(mockdao);
    ...
    public void LoginTest() throws Exception   {
    	...
    	PowerMockito.doReturn(listRegistrForm).when(mockdao).getCustomRecord("",email+password,email );
    	serviceRegisterForm = testService.Login(email, password, 0);
    	LOG.info(serviceRegisterForm.get(0).toString());
          assertTrue("Chesk ro Login registation Visa Form ",serviceRegisterForm.equals(listRegistrForm));
    }
